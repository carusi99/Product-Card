          ...
          <li key={product.id} className="border-b border-gray-300 py-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                <img src={product.image_url} alt={product.name} className="w-12 h-12 object-cover mr-4" />
                <div>
              ...
              <div className="flex items-center">
                <p className="text-gray-700 mr-4">${product.price * product.quantity}</p>
                <button className="text-red-500 hover:text-red-700" onClick={() => removeFromCart(product.id)}>Eliminar</button>
              </div>
            </div>
          </li>
        ))}
